...
embeddings = DashScopeEmbeddings(
    model="text-embedding-v3", dashscope_api_key=os.getenv("QWEN_API_KEY")
)
rag_vectordb = "/root/autodl-tmp/multimodel_RAG/faiss_index"
vector_store = FAISS.load_local(
    folder_path=rag_vectordb,
...
# %%
SYSTEM_INSTRUCTION = """
你是一个中国大陆旅游向导，你善于回答回答中国内陆各省市旅游相关的一切问题。
...
async def grade_search_docs(state: State, evaluate_model=model) -> Literal[
    ...
    if state['generate_times']>2:
        ...
        return 'generate_final_answer'
    print(f"进入rag_agent打分节点,当前生成次数{state["generate_times"]}，"
